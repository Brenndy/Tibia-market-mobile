...
export async function mockAllApis(page: Page) {
  ...
  await page.route('**/api/tibia/item_metadata**', (route) =>
  ...
  await page.route('**/api/tibia/world_data**', (route) =>
    route.fulfill({ json: worldData })
  );

  // TibiaData worlds (pvp_type + battleye)
  await page.route('**/tibiadata.com/**', (route) =>
    route.fulfill({ json: tibiaDataWorlds })
  );

  // Item price history
  await page.route('**/api/tibia/item_history**', (route) =>
    route.fulfill({ json: itemHistory })
  );

  // Item offers (market board)
  await page.route('**/api/tibia/market_board**', (route) =>
    route.fulfill({ json: itemOffers })
  );
